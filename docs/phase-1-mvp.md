...
Goal: Build the minimal, high-quality mobile experience that enables discovery, event details, event
submission with moderation, saved events and reminders, and the moderator/admin back-office.
...
Goal: Build the minimal, high-quality mobile experience that enables discovery, event details,
event submission with moderation, saved events and reminders, and the moderator/admin back-
office.
...
- Denormalize small snapshots on `events` (categoryName, organizerName, venueName, posterThumb) for
	efficient lists.
- Implement server-side slug mapping to ensure shareable and relatively stable URLs.
- Use geohash-based bounding box approach (or geofirex) for radius queries.
...
- Start with coarse price tiers and add numeric price fields in schema for Phase 2
